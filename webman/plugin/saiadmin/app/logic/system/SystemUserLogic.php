...
class SystemUserLogic extends BaseLogic
{
     ...
     * @param string $type
     * @param string $googleCode 谷歌验证码
     * @return array
     */
    public function login(string $username, string $password, string $type, string $googleCode = ''): array
    {
        ...
        // 谷歌验证码验证
        if ($adminInfo->google_auth_enabled) {
            if (empty($googleCode)) {
                throw new ApiException('请输入谷歌验证码');
            }
            $googleAuthLogic = new \plugin\saiadmin\app\logic\GoogleAuthLogic();
            if (!$googleAuthLogic->verifyCode($adminInfo->google_secret, $googleCode)) {
                // 登录事件
                Event::emit('user.login', compact('username','status','message'));
